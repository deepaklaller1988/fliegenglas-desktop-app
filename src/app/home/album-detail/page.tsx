...
export default function AlbumDetail() {
    ...
    //   }
      try {
        const response = API.get(`getProductByID?product_id=${id}&customerID=${user.id}`);
        return response || {};
      } catch (error) {
        console.log(error);
        return {}
    ...
    return (
        <div className='rightSideSet'>
            <div className='loaderSet w-full h-full items-center justify-center hidden'>
                <Image className="block w-full max-w-[150px]" width={265} height={300} src={""} alt="Album" />
            </div>
            <div className='w-full h-full overflow-auto bgChangeAlbum bg-cover bg-center bg-fixed' style={{ backgroundImage:"/" + data.local_image }}>
                <div className='w-full p-3 relative z-10'>
                    ...
                    <div className='w-full'>
                        <Image className="block w-full shadow-xl" src={"/" + data?.local_image} alt="Album" width={265} height={300} />
                    </div> 
                 ...
                 <div className='w-full bg-white/80 rounded-md p-3 mt-3'>
                    ...
                    <div className='w-full bg-white/80 rounded-md p-3 mt-3 flex gap-3'>
                        <span className='min-w-[85px] max-w-[85px]'>
                            <Image className="block w-full" src={"/" + data.authoravatar} alt="Author" width={85} height={85} />
                        </span>
                    ...
                    <div className='w-full bg-white/80 rounded-md p-3 mt-3 flex gap-3'>
                        <span className='min-w-[85px] max-w-[85px]'>
                            <Image className="block w-full" src={"/" + data?.artistavatar } alt="Speaker" width={85} height={85} />
                        </span>
                        <span className='relative w-full'>
                            <b className='text-[#232a2c] text-[16px]'>Sprecher*ln:</b>
                             <p className='text-[#232a2c] text-[16px] opacity-60 leading-none'>{data?.artist}</p> 
                            <Link className='absolute bottom-0 right-0 bg-[#6c7279] text-white w-[128px] p-[5px] px-2 rounded-md text-sm text-center' href="/artist-details">Alle Hörbücher</Link>
                        </span>
                    </div>
                    <div className='w-full bg-white/80 rounded-md p-3 py-8 mt-3 flex gap-4 flex-col'>
                        <h2 className='text-center text-[#232a2c]'>Inhalt des Hörbuches</h2>
                         <p className='text-[#232a2c] leading-6'> <ProductDes data={data}/></p>                 
                    </div>
                </div>
            </div>
        </div>
