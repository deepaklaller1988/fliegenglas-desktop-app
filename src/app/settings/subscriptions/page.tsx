                ...
                <div className="max-w-md mx-auto bg-gray-900 text-white rounded-lg overflow-hidden shadow-lg">
                    <div className="p-4 flex">
                        ...
                        <div className="ml-4">
                            ...
                            <p className="mt-2 text-sm">{item?.subscription_period}: {item?.amount}€</p>
                            <p className="text-sm">Wird am <span className="font-semibold">{item?.subscription_till}</span> verlängert.</p>
                            <p className="text-sm mt-2">Zahlungsart: {item?.method}</p>
                        </div>
                    </div>
                    <button onClick={() => setShowModal(true)} className="w-full py-3 bg-gray-800 hover:bg-gray-700 text-center text-sm font-semibold rounded-b-lg">
                        Abo Kündigen
                    </button>
                </div>
            ))}
            {showModal && <ErrorPopup message={"Um dieses Abo zu künden, gehen Sie bitte in die Einstellungs-App Ihres Mobile-Telefons, klicken Sie auf Ihren Namen, dann unter Abonnemente und künden Sie das Fliegenglas Abo."}
