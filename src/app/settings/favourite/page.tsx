  ...
  const mutation: any = useMutation({
    mutationFn: async () => {
      const response = await API.post(
        `addOnFavPage?&channel_id=${selectedChannels}&user_id=${
          user.id
        ...
        <div className="w-full">
          ...
          <div>
            <p className="text-lg text-white mb-4 mt-4 ml-4">
              Wähle die Kategorien oder Kanäle aus, die Du auf der Startseite
              angezeigt bekommen möchtest.
            </p>

            <div className="flex items-center justify-center w-full">
              <button
                onClick={handleFavCat}
                className={`rounded-md h-[50px] bg-[#ff9900] px-20 hover:bg-[#ff6600] ${
                  mutation.isPending ? "flie-loader !bg-[#ff9900]" : ""
                }`}
